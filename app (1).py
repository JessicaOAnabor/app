...
token = st.query_params.get("token", [""])[0]   # use new API
...
try:
    ...
    xi = float(np.clip(xi, -0.9, 0.9))  # clip to a safe range
...
if xi > 0:
    try:
        ...
        st.warning("Pareto sampling failed, falling back to empirical fraction.")
        mc_p = frac
else:
    # Fallback when Pareto is invalid
    mc_p = frac
