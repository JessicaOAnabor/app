...
if run_btn:
    ...
    with tab1:
        ...
        # 2) Return-Period Plot (interactive hover)
        xi = result["xi"]
        scale = result["scale"]
        ...
        fig_rp.add_scatter(
            ...
            line=dict(color="red"),
            hovertemplate=(
                "<b>GPD Fit</b><br>Value: %{x:.2f}<br>RP: %{y:.1f}<extra></extra>"
            ),
        )
        fig_rp.update_traces(
            selector=dict(mode="lines+markers"),
            hovertemplate=(
                "<b>Empirical</b><br>Value: %{x:.2f}<br>RP: %{y:.1f}<extra></extra>"
            ),
        )
        st.plotly_chart(fig_rp, use_container_width=True)

        # 3) Data Histogram with Tail Fit (interactive hover)
        ...
        fig_hist.add_scatter(
            ...
            hovertemplate=(
                "<b>GPD PDF</b><br>Value: %{x:.2f}<br>Density: %{y:.1f}<extra></extra>"
            ),
        )
