...
if run_btn:
    ...
    with tab1:
        ...
        fig_rp = px.line(
            ...
            title="Return-Period Plot (hover for details)",
        )
            # Overlay GPD Fit
        fig_rp.add_scatter(
            x=xs,
            ...
            name="GPD Fit",
            line=dict(color="red"),
            hovertemplate="<b>GPD Fit</b><br>Value: %{x:.2f}<br>RP: %{y:.1f}<extra></extra>",
        )

    # Customize primary trace hover
        fig_rp.update_traces(
            selector=dict(mode="lines+markers"),
            hovertemplate="<b>Empirical</b><br>Value: %{x:.2f}<br>RP: %{y:.1f}<extra></extra>"
        )
        ...
        fig_hist = px.histogram(
            ...
            title="Data Histogram with Tail-Fit (hover for breakdown)",
        )
    # Add tail‐fit line
        tail_x = xs[xs > thr]
        ...
        fig_hist.add_scatter(
            ...
            hovertemplate="<b>GPD PDF</b><br>Value: %{x:.2f}<br>Density: %{y:.1f}<extra></extra>",
       )

    # Enable richer hover on bars
      fig_hist.update_traces(
          hovertemplate="<b>Bin center</b>: %{x:.2f}<br>Count: %{y}<extra></extra>"
      )
    ...
    with tab2:
        ...
        fig_shap.update_traces(
            hovertemplate=(
                "<b>Year</b>: %{x}<br>"
                "<b>Contribution</b>: %{y:.3f}<br>"
                "<b>Abs. Rank</b>: %{customdata}"
                "<extra></extra>"
            ),
            customdata=list(df_shap["Contribution"].abs().rank(ascending=False).astype(int))
        )
    ...
    with tab4:
        ...
        # Enrich hover: show scenario, metric name, baseline & pilot
        fig_kpi.update_traces(
            hovertemplate=(
                "<b>Metric</b>: %{x}<br>"
                "<b>Scenario</b>: %{legendgroup}<br>"
                "<b>Value</b>: %{y:.4f}"
                "<extra></extra>"
            ),
        ...
        # Enrich hover: show year, scenario, and cumulative cost
        fig_save.update_traces(
            hovertemplate=(
                "<b>Year</b>: %{x}<br>"
                "<b>Scenario</b>: %{legendgroup}<br>"
                "<b>Cumulative Cost</b>: $%{y:.2f}"
                "<extra></extra>"
            )
        )
