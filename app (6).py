...
if st.sidebar.button("Run Demo QPO", key="sidebar_demo_qpo"):
...
if run_btn:
    ...
    with tab2:
        ...
        # 3) Counterfactual UI
       # Counterfactual slider + number_input with consistent float type
        st.markdown("---")
        years_int = df_shap["Year"].tolist()
        ...
        default_val = float(series.loc[cf_year])
        cf_val = st.number_input(
            f"New value for {cf_year}",
            value=default_val,
            min_value=0.0,
            step=0.1,
            key="cf_val_input"
        ...
        if st.button("Run Counterfactual", key="cf_run_btn"):
            try:
                ser_int = series.copy()
    ...
    with tab3:
        st.subheader("Demo QPO: Quantum Capital Allocator")
        ...
        samples = st.slider(
            ...
            5, 50, 20, 5,
            key="m_samples_slider"
        )

        st.markdown("---")
        if st.button("Run Demo QPO", key="tab3_demo_qpo_run"):
            try:
                exps = [float(x.strip()) for x in exps_input.split(",") if x.strip()]
                if not exps:
                    raise ValueError("Provide at least one exposure.")
                if budget <= 0:
                    raise ValueError("Budget must be positive.")

                exps = np.array(exps)
                def objective(a): return np.sum(np.sqrt(exps * a))
                ...
                for _ in range(samples):
                    ...
                    if score > best_obj:
                        best_obj, best_alloc = score, cand
                ...
                st.download_button(
                    "Download Demo QPO Audit Log",
                    data=csv,
                    file_name=f"demo_qpo_audit_{country}.csv",
                    key="tab3_demo_qpo_dl"
                )
            except Exception as e:
    ...
    with tab5:
        ...
        # 2) Compute empirical VaR
        thr_emp = float(np.percentile(series, conf / 100))

        # 3) Compute GPD VaR if valid
        var_gpd = thr_emp
        if result["xi"] > 0:
            p = 1 - conf / 100
            xi, scale, thr0 = result["xi"], result["scale"], result["thr"]
