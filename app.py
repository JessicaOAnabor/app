...
def tail_risk_analysis(series):
    ...
    for _ in range(200):
        ...
        eb   = samp[samp>tb] - tb
        if len(eb)>0: boot_ps.append(len(eb)/len(samp))
    ...
    return dict(pct=pct, thr=thr, xi=xi, scale=scale,
                p_lo=p_lo, p_hi=p_hi,
                mc_p=mc_p, t_mc=t_mc,
                q_p=q_p, lo_q=lo_q, hi_q=hi_q, t_q=t_q,
                q_err=q_err, speed=speed)
